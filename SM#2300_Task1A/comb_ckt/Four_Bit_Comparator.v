...
Two_Bit_Comparator m1( .A(A[3:2]) , .B(B[3:2]) , .A_Greater(b1) , .Equal(b2) , .B_Greater(b3));

Two_Bit_Comparator m0( .A(A[1:0]) , .B(B[1:0]) , .A_Greater(c1) , .Equal(c2) , .B_Greater(c3));

and an1( Equal, b2 , c2);

assign A_Greater = b1 | (a4 & a3 & (c1));

assign B_Greater = b3 | (a4 & a3 & (c3)); 
